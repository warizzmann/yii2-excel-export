 ...
 *
 * Based on BSOURCECODE's GridView To Excel Export Extension In Yiiframework 2.0
 * @see http://www.bsourcecode.com/yiiframework2/gridview-to-excel-export-extension-in-yiiframework-2-0/
 *
 * @author Warisman Syam <warisman@docotel.co.id>
 * @version 0.1
 */

class ExcelExport extends \yii\grid\GridView
{
    ...
    public function init_excel_sheet()
    {
        ...
        $this->_objPHPExcel->getProperties()
            ...
            ->setSubject($this->properties['subject'])
            ->setDescription($this->properties['description'])
            ->setCategory($this->properties['category'])
            ->setKeywords($this->properties['keywords'])
            ->setManager($this->properties['manager'])
            ->setCompany($this->properties['company'])
            ->setCreated($created)
            ->setLastModifiedBy($lastModifiedBy);
        $this->_objPHPExcelSheet = $this->_objPHPExcel->getActiveSheet();
        if (isset($this->sheet_properties['showGridlines'])) {
            $this->_objPHPExcelSheet->setShowGridlines(!!$this->sheet_properties['showGridlines']);
        }
    }
    ...
    public function generateHeader()
    {
        ...
        if ($this->sheet_header) {
            $header_cell = $sheet->setCellValue('A1', $this->sheet_header['value']);
            if (isset($this->sheet_header['merge'])) {
                $mergeCells = 'A1:'.self::columnName((int)$this->sheet_header['merge']).'1';
                $header_cell->mergeCells($mergeCells)->getStyle($mergeCells)->applyFromArray([
                        'alignment' => ['horizontal' => \PHPExcel_Style_Alignment::HORIZONTAL_CENTER],
                        'font' => ['bold' => true]
                    ]
                );
            } else {
                $header_cell->mergeCells('A1:C1')->getStyle('A1:C1')->applyFromArray([
                        'font' => ['bold' => true]
                    ]
                );
            }
    ...
    public function generateRow($model, $key, $index)
    {
        ...
        foreach ($this->_visibleColumns as $column) {
            ...
            $cell = $this->_objPHPExcelSheet->setCellValue(self::columnName($this->_endCol) . ($index + $this->_beginRow + 1),
                strip_tags($value));
            $this->_objPHPExcelSheet->getColumnDimension(self::columnName($this->_endCol))->setAutoSize(true);
            // $cell->getStyle(self::columnName($this->_endCol) . ($index + $this->_beginRow + 1))->applyFromArray([
            //         'alignment' => ['horizontal' => \PHPExcel_Style_Alignment::HORIZONTAL_CENTER],
            //         'font' => ['bold' => true]
            //     ]
            // );
